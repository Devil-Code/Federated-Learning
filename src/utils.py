...
def train(model, train_loader, validate_loader, num_examples,partition, device="cuda",  log_interval = 100, epochs = 10, es_patience = 3):
    # Training model
    ...
    for e in range(epochs):
        ...
        val_loss, val_auc_score, val_accuracy, val_f1 = val(model, validate_loader, criterion, partition, device)
...
def val(model, validate_loader, criterion, partition, device="cuda"):          
    model.eval()
    preds=[]            
    all_labels=[] 
